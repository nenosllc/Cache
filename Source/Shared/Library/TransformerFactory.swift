...
public class TransformerFactory {
    ...
    public static func forImage() -> Transformer<CacheImage> {
        let toData: (CacheImage) throws -> Data = { image in
            return try image.cache_toData().unwrapOrThrow(error: StorageError.transformerFail)
        }
        
        let fromData: (Data) throws -> CacheImage = { data in
            return try CacheImage(data: data).unwrapOrThrow(error: StorageError.transformerFail)
        }
        
        return Transformer<CacheImage>(toData: toData, fromData: fromData)
    }
    #endif
